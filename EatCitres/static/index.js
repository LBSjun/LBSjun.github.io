...
(function(w) {
    ...
    function shareText(score) {
        ...
        if (mode === MODE_ENDLESS) {
            if (score <= 5) return 'Ci';
            if (score <= 8) return 'Citr';
            if (score <= 10)  return 'Citres';
            return 'CITRESSS';
        } else {
            if (score <= 49) return 'Ci';
            if (score <= 99) return 'Citr';
            if (score <= 149) return 'Citres';
            return 'CITRESSS';
        }
    }
    ...
    function initSetting() {
        ...
        document.getElementsByTagName("title")[0].innerText = cookie("title") ? cookie("title") : "吃掉蕾丝";
        if (cookie("keyboard")) {
